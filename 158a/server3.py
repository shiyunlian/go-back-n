...
last_ack_sent = packet_num % limit # calculate the last ack sent based on the packet number and sequence number limit

all_packets_received = False
track_resent_packets_buffer = []    # an array to keep track of how many times each packet sent
first_resent_packet = 0
last_resent_packet = 0
...
while countdown > 0 and all_packets_received == False:
    ...
    # if there are packets dropped, send the last received packet cumulative ack and shrink window size
        ...
        first_resent_packet = last_received_packet[0]+1
        last_resent_packet = recent_packet
